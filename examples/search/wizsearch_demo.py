...
Prerequisites:
- Set up API keys fowizagent.search.duckduckgo_searchr Tavily, Google AI, etc. as needed
...
# Import WizSearch components
from wizagent.search import WizSearch, WizSearchConfig
...
async def demo_registry_features():
    ...
    wiz_search = WizSearch()
    print(f"Auto-enabled engines: {wiz_search.get_enabled_engines()}")
...
async def demo_basic_search():
    ...
    print(f"Auto-enabled engines: {wiz_search.get_enabled_engines()}")
    print(f"Configuration: {json.dumps(wiz_search.get_config(), indent=2)}")
    ...
    result = await wiz_search.async_search(query)
...
async def demo_custom_configuration():
    ...
    config = WizSearchConfig(
        ...
        enabled_engines=["duckduckgo", "searxng"],  # Only specific engines
    ...
    print(f"Custom enabled engines: {wiz_search.get_enabled_engines()}")
    print(f"Custom configuration: {json.dumps(wiz_search.get_config(), indent=2)}")
...
async def demo_synchronous_search():
    """Demonstrate synchronous search functionality."""
    print_separator("Synchronous Search Demo")

    # Create WizSearch for sync demo
    config = WizSearchConfig(
    ...
    print(f"Synchronous search for: '{query}'")

    start_time = time.time()
    result = wiz_search.search(query)  # Synchronous call
    search_time = time.time() - start_time

    print_search_results(result, "Synchronous Search Results")
    print(f"Total search time: {search_time:.2f}s")
...
async def demo_merge_behavior():
    ...
    config = WizSearchConfig(enabled_engines=["duckduckgo", "searxng"], max_results_per_engine=8, timeout=20)
    wiz_search = WizSearch(config=config)
...
async def demo_error_handling():
    ...
    try:
        ...
        wiz_search = WizSearch(config=config)
        await wiz_search.async_search("test query")
    except Exception as e:
    ...
    result = await wiz_search.async_search("complex query that might timeout")
...
async def demo_performance_comparison():
    ...
    from wizagent.search import DuckDuckGoSearch, DuckDuckGoSearchConfig
    ...
    start_time = time.time()
    single_result = await single_engine.async_search(query)
    ...
    config = WizSearchConfig(enabled_engines=["duckduckgo", "searxng"], max_results_per_engine=5, timeout=15)
    wiz_search = WizSearch(config=config)
    start_time = time.time()
    wiz_result = await wiz_search.async_search(query)
