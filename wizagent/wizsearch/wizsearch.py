...
from wizagent.search.base import BaseSearch, SearchResult
...
class SearchEngineRegistry:
    ...
    def _discover_engines(cls) -> None:
        ...
        engine_modules = [
            ("wizagent.search.duckduckgo_search", "DuckDuckGoSearch", "DuckDuckGoSearchConfig"),
            ("wizagent.search.tavily_search", "TavilySearch", "TavilySearchConfig"),
            ("wizagent.search.google_genai_search", "GoogleAISearch", None),
            ("wizagent.search.searxng_search", "SearxNGSearch", "SearxNGSearchConfig"),
        ]
...
class WizSearch(BaseSearch):
    ...
    async def _search_engine_with_timeout(
        ...
        try:
            ...
            result = await asyncio.wait_for(engine.async_search(query, **kwargs), timeout=self.config.timeout)
    ...
    def search(self, query: str, **kwargs) -> SearchResult:
        """
        Perform a synchronous search query over all integrated engines.
        ...
        """
        # Run async search in a synchronous context
        ...
        if loop.is_running():
            ...
            def run_async():
                ...
                try:
                    return new_loop.run_until_complete(self.async_search(query, **kwargs))
                finally:
                    new_loop.close()
            ...
            return loop.run_until_complete(self.async_search(query, **kwargs))

    async def async_search(self, query: str, **kwargs) -> SearchResult:
        """
        Perform an async search query over all integrated engines concurrently.
