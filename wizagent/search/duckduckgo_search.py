...
class DuckDuckGoSearch(BaseSearch):
    ...
    def _transform_results(self, query: str, raw_results: list, response_time: float) -> SearchResult:
        ...
        for result in raw_results:
            source_item = SourceItem(
                ...
                score=None,  # DDGS doesn't provide relevance scores
                raw_content=None,
            )
    ...
    async def async_search(self, query: str, **kwargs) -> SearchResult:
        ...
        try:
            ...
            logger.info(f"Performing async DuckDuckGo search for query: {query}")
            loop = asyncio.get_event_loop()

            # Create a partial function with kwargs to pass to run_in_executor
            from functools import partial

            search_func = partial(self.search, query, **kwargs)
            result = await loop.run_in_executor(None, search_func)

            response_time = (datetime.now() - start_time).total_seconds()
            logger.info(f"Async DuckDuckGo search completed in {response_time:.2f} seconds")
            return result

        except Exception as e:
            logger.error(f"DuckDuckGo async search failed for query '{query}': {e}")
            raise DuckDuckGoSearchError(f"Async search failed: {e}")
