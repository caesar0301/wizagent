...
class SearxNGSearch(BaseSearch):
    ...
    def _transform_results(self, query: str, raw_results: SearxNGResults, response_time: float) -> SearchResult:
        ...
        for result in raw_results.results[: self.config.max_results]:
            source = SourceItem(
                url=result.get("url", ""),
                title=result.get("title", ""),
                content=result.get("content", ""),
                score=None,  # SearxNG doesn't provide relevance scores
    ...
    def search(self, query: str, **kwargs) -> SearchResult:
        ...
        try:
            ...
            raw_results = self._searx_api_query(params)
    ...
    async def async_search(self, query: str, **kwargs) -> SearchResult:
        """
        Perform an async search query using SearxNG.
        ...
        try:
            logger.debug(f"Performing async SearxNG search for query: {query}")
            ...
            logger.debug(f"Async SearxNG search completed in {response_time:.2f}s with {len(result.sources)} results")
            ...
            logger.error(f"Unexpected error in async SearxNG search: {e}")
            raise SearxNGSearchError(f"Unexpected error in async SearxNG search: {e}")
